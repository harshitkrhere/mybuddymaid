// supabase/functions/send-booking-email/handler.ts
...
// caller check at all. The recipient and the email_logs attribution now come from the caller's
// token; only the booking's own details are still read from the body.
...
// `user_id` and `user_email` are deliberately NOT required fields any more, and are ignored
// if sent: they come from the caller's token (FIN-S01, FIN-S08). Everything still read from
// the body describes the booking, not the person, and the email only ever goes to the token
// owner — so the worst a forged value can do is misdescribe a booking to its own owner.
function validatePayload(body: Record<string, unknown>, user: User): {
  valid: boolean;
  error?: string;
...
export async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    // Validate payload
    const validation = validatePayload(body, user);
