// supabase/functions/send-package-email/handler.ts
...
//
// FIN-S01: this function used to send to whatever address the request body named, with no
// caller check at all. It now authenticates the caller and reads every identity and plan
// value from the token and the database — the body is not trusted for anything.
...
export async function handler(req: Request): Promise<Response> {
  ...
  try {
    // ── Verify the caller is authenticated (FIN-S01) ──
    ...
    if (!user.email) {
      // Nothing to send to. Only reachable for an account created without an email address.
      return jsonResponse({ error: 'This account has no email address' }, 422);
    }

    // ── Read the plan from the database, not the request ──
    // The body is ignored entirely. Trusting it for plan_name/amount_paid/expires_at would
    // let any authenticated user mail themselves a genuine-looking Diamond confirmation, and
    // would let a forged user_id pollute email_logs (FIN-S08).
    const { data: plan, error: planError } = await supabaseAdmin
      .from('user_plans')
      .select('plan_name, amount_paid, razorpay_payment_id, replacements_total, purchased_at, expires_at')
      .eq('user_id', user.id)
      .eq('is_active', true)
    ...
    if (planError) {
      console.error('[send-package-email] Could not read user_plans:', planError);
      return jsonResponse({ error: 'Could not read your plan' }, 500);
    }

    if (!plan) {
      // The caller is who they say they are but has no plan to confirm. Callers invoke this
      // immediately after verify-razorpay-payment returns, so in the normal flow the row is
      // already committed; this is the honest answer when it is not.
      return jsonResponse({ error: 'No active plan found for this account' }, 404);
    }

    // full_name lives on profiles, not user_plans, and is null until onboarding completes.
    ...
    payload = {
      user_name: profile?.full_name?.trim() || 'Valued Customer',
      user_email: user.email,
      user_id: user.id,
      plan_name: plan.plan_name,
      amount_paid: plan.amount_paid,
      razorpay_payment_id: plan.razorpay_payment_id ?? '',
      purchased_at: plan.purchased_at,
      replacements_total: plan.replacements_total,
