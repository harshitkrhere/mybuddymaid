/**
 * MyBuddyMaid — Google Apps Script (Complete Rebuild)
 * ═══════════════════════════════════════════════════════
 * Routes form submissions to two separate tabs:
 *   • "Bookings"    ← Instant Book form (general service enquiries)
 *   • "Enrollments" ← Package signups (Silver / Gold / Diamond / Platinum)
 *
 * Auto-sends a branded HTML confirmation email + PDF invoice
 * after each enrollment via Resend API.
 * ═══════════════════════════════════════════════════════
 */
...
const PLANS = {
  2499: {
    emoji: '🥈', color: '#64748b', tag: 'Silver Buddy',
    validity: '90 Days', contacts: '1 Verified Profile',
    features: [
  ...
  3999: {
    emoji: '🥇', color: '#d97706', tag: 'Gold Buddy',
    validity: '180 Days', contacts: '3 Verified Profiles',
    features: [
  ...
  5999: {
    emoji: '💎', color: '#0d9488', tag: 'Diamond Buddy',
    validity: '365 Days', contacts: '5 Verified Profiles',
    features: [
  ...
  8999: {
    emoji: '👑', color: '#7c3aed', tag: 'Platinum Buddy',
    validity: '456 Days', contacts: '15 Replacements Included',
    features: [
      '456-Day Replacement Guarantee',
      'Up to 15 Free Replacements',
      'VIP Concierge Verified Pool',
      'Full Police + Medical Background Check',
      '24×7 Priority Account Manager',
      'Same-Day Emergency Deployment',
      'Full Refund + Insurance + Extended Cover',
    ],
    teamMsg: 'Your VIP concierge manager will personally call you within <strong>15 minutes</strong> with our top-tier buddy profiles for immediate same-day deployment.',
  },
};

/** Look up plan by amount, with fallback */
function getPlan(amount) {
  if (PLANS[amount]) return PLANS[amount];
  // Threshold-based fallback
  if (amount <= 2499) return PLANS[2499];
  if (amount <= 3999) return PLANS[3999];
  if (amount <= 5999) return PLANS[5999];
  return PLANS[8999];
}

/** Resolve plan name from amount */
function resolvePlanName(amount) {
  return getPlan(amount).tag;
}
...
function doPost(e) {
  try {
    ...
    // ── 1. Razorpay Backend Webhook ──
    if (data.event === 'payment.captured') {
      const payment  = data.payload.payment.entity;
      const amount   = payment.amount / 100;
      const planName = payment.notes?.package || resolvePlanName(amount);
      const utr      = payment.id;

      const sheet = ss.getSheetByName('Enrollments');
      const rows  = sheet.getDataRange().getValues();
      const isDuplicate = rows.some(row => String(row[6]) === utr);

      if (!isDuplicate) {
        sheet.appendRow([
          new Date().toLocaleString('en-IN'),
          payment.notes?.name || 'Customer',
          payment.email || '',
          "'" + (payment.contact || ''),
          planName,
          amount,
    ...
    if (data.type === 'enrollment') {
      ...
      const utr   = data.utr || '';
      const rows  = sheet.getDataRange().getValues();
      const isDuplicate = utr ? rows.some(row => String(row[6]) === utr) : false;
...
function sendEnrollmentEmail(data) {
  ...
  const { emoji, color, tag, validity, contacts, features, teamMsg } = plan;
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#f8fafc;border-radius:12px;overflow:hidden;margin-bottom:24px;">
      ...
      <tr><td style="padding:16px;">
        <table width="100%" cellpadding="0" cellspacing="0" style="font-size:14px;">
          ...
          <tr><td style="color:#64748b;padding:6px 0;">Amount</td><td style="color:${color};font-weight:700;text-align:right;font-size:16px;">${amtFormatted}</td></tr>
  ...
  try {
    ...
    if (response.getResponseCode() >= 400) {
      ...
      ss.getSheetByName('Bookings').appendRow([
        new Date().toLocaleString('en-IN'), 'EMAIL ERROR', email,
...
/* ──────────────────────────────────────────────── *
 *  Setup — Run once to create sheet tabs
 * ──────────────────────────────────────────────── */
